...
class FluxoCaixaModel extends Model
{
    ...
    function listar()
    {
        ...
        try {
            $qry = "SELECT lc.id, lc.idusuario, lc.idcaixa, lc.dataemissao,
                ...
                lc.id_movimento, lc.vencimento, lc.detalhes FROM lancamentoscaixa lc
                WHERE lc.situacao = 'S' ORDER BY lc.id DESC LIMIT 30";
    ...
    function pesquisar($texto)
    {
        ...
        try {
            $qry = "SELECT lc.id, lc.idusuario, lc.idcaixa, lc.dataemissao,
                ...
                lc.id_movimento, lc.vencimento, lc.detalhes FROM lancamentoscaixa lc
                WHERE lc.situacao = 'S' AND lc.observacoes LIKE '%" . $texto . "%'            
    ...
    function partial($qt)
    {
        ...
        try {
            $qry = "SELECT lc.id, lc.idusuario, lc.idcaixa, lc.dataemissao,
                lc.tipomovimento, lc.ndoc, lc.valor, lc.cadastro, lc.observacoes, 
                lc.parcela, lc.id_origem, lc.sigla_origem, lc.hora, lc.situacao, 
                c.descricao_recebimento, lc.id_plano_contas,
                lc.id_movimento, lc.vencimento, lc.detalhes FROM lancamentoscaixa lc
                WHERE lc.situacao = 'S' ORDER BY lc.id DESC LIMIT " . $qt;
            $stmt = $this->conexao->prepare($qry);
            $stmt->execute();
    ...
    function view($id)
    {
        ...
        try {
            $qry = "SELECT lc.id, lc.idusuario, lc.idcaixa, lc.dataemissao,
                ...
                lc.descricao_recebimento, lc.id_plano_contas,
                lc.id_movimento, lc.vencimento, lc.detalhes FROM lancamentoscaixa lc
                WHERE lc.situacao = 'S' AND lc.id = '$id'";
            $stmt = $this->conexao->prepare($qry);
            $stmt->execute();
    ...
    function inserir($data)
    {
        ...
        try {
            ...
            $stmt->bindValue("p23", $data_pagamento);
            $stmt->bindValue("p24", $hora_pagamento);
    ...
    function contaTotalCaixa()
    {
        ...
        try {
            $qry = "SELECT COUNT(*) AS totalCaixa FROM lancamentoscaixa";
            $stmt = $this->conexao->prepare($qry);
            $stmt->execute();
            ...
            foreach ($results as $row) {
                $data = array(
                    "total" => $row['totalCaixa'],
                    "request" => [
                        "type" => "GET",
                        "description" => "Retorna todos os registros",
                        "url" => "api/fluxocaixa/totalcp",
                    ]
                );
            }
        } catch (\Exception $e) {
